...
//           const response = await axios.get(`https://piserver-ljd1.onrender.com/userTasks/${taskId}/${id}`);
...
const TaskList = () => {
  ...
  const [lastAlertTime, setLastAlertTime] = useState(
    localStorage.getItem("lastAlertTime")
  );
  const [authorizationChecked, setAuthorizationChecked] = useState(false);
  ...
  return (
    <div>
      ...
      {authorizationChecked ? (
        token && data && data.is_active ? (
          <div>
            <h6 className="text-center text-info fw-700 m-3">
              Hello, {data.name}
            </h6>
            <h5 className="text-secondary text-center fw-600 m-1">
              Today's Task
            </h5>

            {allTasksCompleted && (
              <h6 style={{ marginLeft: "20px", color: "green" }}>
                ...
                {index === 0 || checkPreviousTaskCompletion(index) ? (
                  ...
                  >
                    <div className="task_container">
                      ...
                      <div className="task_box">
                        <h6 className="text-light" style={{ width: "200px" }}>
                          {task.title}
                        </h6>
                        <img
