...
const ContactInfo = () => {
  ...
  useEffect(() => {
    ...
    axios.get('https://mlm-gc1b.onrender.com/api/contactInfo').then((response) => setUserDetails(response.data));
  }, []);
  ...
  const handleSave = () => {
    ...
    axios[requestMethod]('https://mlm-gc1b.onrender.com/api/contactInfo', userDetails).then(() => {
      setIsEditMode(false);
    });
  };
