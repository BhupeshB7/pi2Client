...
function  GameDeposit() {
  ...
  useEffect(() => {
    ...
    fetch('https://mlm-gc1b.onrender.com/api/statistics') // Assuming your backend is running on the same host/port
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://mlm-gc1b.onrender.com/api/deposit/history?page=${currentPage}`
        );
  ...
  const handleApprove = async (id, amount) => {
    ...
    try {
      const response = await axios.put(`https://mlm-gc1b.onrender.com/api/approve/${id}`, { amount });
  ...
  const deleteDeposit = async (id) => {
    try {
      const response = await axios.delete(
        `https://mlm-gc1b.onrender.com/api/gameDeposit/delete/${id}`
      );
