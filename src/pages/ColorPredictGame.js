...
const ColorPredictGame = () => {
  ...
  const fetchSessionInfo = async () => {
    try {
      const response = await axios.get(
        "https://piserver-ljd1.onrender.com/three/getLatestSession"
      );
  ...
  const fetchTimer = async () => {
    try {
      const response = await axios.get(
        `https://piserver-ljd1.onrender.com/three/api/user/getTimer/${sessionInfo.sessionId}`
      );
  ...
  const getGamerProfile = async () => {
    try {
      const response = await axios.get(
        `https://piserver-ljd1.onrender.com/api/gameProfile/${data.userId}`
      );
  ...
  if(remainingTime===2){
    try {
      const response = axios.post('https://piserver-ljd1.onrender.com/three/MinuteuserResult',{
    ...
    if (betAmount < 1) {
      ...
      try {
        const response = await axios.post(
          "https://piserver-ljd1.onrender.com/three/oneMinuteHistory",
          {
            userId: data.userId,
