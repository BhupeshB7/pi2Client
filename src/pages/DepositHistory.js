...
//           `https://mlm-gc1b.onrender.com/api/deposit/depositHistory/PI45059879`
//         );
...
function DepositHistory() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("https://mlm-gc1b.onrender.com/api/users/profile", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  useEffect(() => {
    if (data && data.userId) {
      const fetchDepositData = async () => {
        try {
          const response = await axios.get(
            `https://mlm-gc1b.onrender.com/api/deposit/depositHistory/${data.userId}`
          );
          const { depositHistory, currentPage, totalPages } = response.data;
